...
def build_linear_svc() -> Pipeline:
    ...
    return Pipeline([
        ...
        (
            "clf",
            LinearSVC(
                class_weight="balanced", random_state=42, max_iter=1000, dual=False
            ),
        ),
    ])
...
def get_top_features(
    pipeline: Pipeline, class_labels: list[str], top_n: int = 20
    ...
    for i, label in enumerate(class_labels):
        try:
            ...
            top_features[label] = [
                (str(feature_names[j]), float(class_coef[j])) for j in top_indices
            ]
        except ValueError:
            pass
